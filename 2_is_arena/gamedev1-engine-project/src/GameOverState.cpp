...
void GameOverState::init(sf::RenderWindow& window) {
...
#pragma region GUISetup
	...
	//Load Image
	Image image;
	image.loadFromFile("../assets/GUI/menu2.png");
	...
	buttonPlay->setPosition(window.getSize().x / 2 - buttonPlay->getSize().x / 2, menu->GetPosition().y + 400);
	buttonPlay->setText("Restart");
	buttonPlay->setTextSize(70);
	buttonPlay->connect("Clicked", [](void) { 
		GameObjectManager::getInstance().reset();
		GameStateManager::getInstance().setState("HeroSelection");
	});
	buttonPlay->focus();
	...
	buttonQuit->connect("Clicked", [](void) {
		GameObjectManager::getInstance().reset();
		PlayFieldManager::getInstance().getScore(0) = 0;
		PlayFieldManager::getInstance().getScore(1) = 0;
	...
	//Add Text
	m_scoretext = make_shared<tgui::Label>();
	m_scoretext->setPosition(tgui::bindPosition(buttonPlay) - Vector2f(170, 200));
	m_scoretext->setTextColor(Color::Black);
	m_scoretext->setTextSize(100);
	m_gui.add(m_scoretext);
...
void GameOverState::update(float delta, sf::RenderWindow& window) {
	m_scoretext->setText(to_string(PlayFieldManager::getInstance()
		.getScore(0)) + "    SCORE    " + to_string(PlayFieldManager::getInstance().getScore(1)));
...
void GameOverState::handleEvents(sf::Event event)
{
